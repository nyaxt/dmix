...
public:
  ...
  template <typename Functor> const Callback &operator=(const Functor &f) {
    static_assert(sizeof(Functor) <= sizeMaxFunctor,
                  "Functor size exceeds max");
    Functor *functorCopy = new (m_memFunctor) Functor(f);
    assert(functorCopy == functor());

    static_assert(sizeof(CallbackEntry<Functor>) <=
                      CallbackEntryInterface::sizeMax,
                  "CallbackEntry size exceeds max");
  ...
  void operator()() { entry()->dispatch(functor()); }
