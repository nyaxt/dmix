...
#define DEFINE_DISPATCH_HANDLER(HANDLER_NAME)                                  \
  ErrorCode_t USBHandler::dispatch##HANDLER_NAME(USBD_HANDLE_T, void *data,    \
                                                 uint32_t event) {             \
...
ErrorCode_t USBHandler::onControl(uint32_t event) { return ERR_USBD_UNHANDLED; }
...
inline void USBHandler::onIRQ() { m_api->hw->ISR(m_handle); }
